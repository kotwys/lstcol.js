...
function classifyGrapheme(grapheme) {
    ...
    for (const [category_, ranges] of Object.entries(CATEGORIES)) {
        ...
        const matches = ranges.some(([start, end]) => codePoint >= start && codePoint <= end);
        if (matches)
            return category;
    }
    return "other";
...
const outputPx = (px) => px.toFixed(3) + "px";
/**
 ...
 */
function splitTokens(el, options) {
    ...
    while (node = walker.nextNode()) {
        ...
        for (const { index, segment } of segmenter.segment(node.textContent)) {
            const category = options?.categoryOverride(segment, el) || classifyGrapheme(segment);
...
function alignElement(el, options) {
